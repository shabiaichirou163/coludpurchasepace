...
public class ItemNewFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        switch (mApplication.getmSeclectFrag()){
            case "oneYuanArea":
                getNewestGoodsInfo(Constants.ONE_AREA_NEW, "new");
                break;
            case "tenYuanArea":
                getNewestGoodsInfo(Constants.TEN_AREA_NEW,  "new");
                break;
            case "oneHundredYuanArea":
                getNewestGoodsInfo(Constants.ONEHUND_AREA_NEW,"new");
                break;
            case "sellerShop":
                break;
                    ...
                    switch (mode){
                        case "new":
                            ...
                            if (null!=mGoodsData&&mGoodsData.size()>0) {
                                ((OneYuanAreaFragment)getParentFragment()).cancelDialog();
                                mNewGrd.setAdapter(mSellerShopAdpater);
                            }
                            break;
                        case "footer":
                            if (null!=mGoodsData&&mGoodsData.size()>0) {
