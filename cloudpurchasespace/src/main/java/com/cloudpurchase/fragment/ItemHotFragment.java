 ...
 * 最热
 */
public class ItemHotFragment extends Fragment {
    private  View mHotFragmentView;
    private MyGridView mHotGrd;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        mHotFragmentView=inflater.inflate(R.layout.fragment_one_yuan_item_hot,null);
        mHotGrd= (MyGridView) mHotFragmentView.findViewById(R.id.one_frag_item_hot);
        ...
        switch (mApplication.getmSeclectFrag()){
            case "oneYuanArea":
                getHotGoodsInfo(Constants.ONE_AREA_HOT,  "new");
                break;
            case "tenYuanArea":
                getHotGoodsInfo(Constants.TEN_AREA_HOT, "new");
                break;
            case "oneHundredYuanArea":
                getHotGoodsInfo(Constants.ONEHUND_AREA_HOT, "new");
                break;
            case "sellerShop":
                break;

        }
        return mHotFragmentView;
    }
 ...
 获取商品详情请求
*/
    public void getHotGoodsInfo(String url,final String mode) {
        //mTag="HttpPost";
                    ...
                    switch (mode){
                        case "new":
                            ...
                            if (null!=mGoodsData&&mGoodsData.size()>0) {
                                ((OneYuanAreaFragment)getParentFragment()).cancelDialog();
                                mHotGrd.setAdapter(mSellerShopAdpater);
                            }
                            break;
                        case "footer":
                            if (null!=mGoodsData&&mGoodsData.size()>0) {
